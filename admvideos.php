...
include('config.php');
$sql = "SELECT vid.*, cur.titulo as curso FROM videos vid LEFT JOIN cursos cur on cur.Id = vid.id_curso";
...
?>
    ...
    <div class="admin-adicionar">
        <button onclick="location.href='admvideoedit.php';"> Adicionar </button>
    </div>
...
?>
    <tr>
        ...
        <td><?=$row['url']?></td>
        <td class="text-end">
            <button onclick="location.href='admvideoedit.php?act=editar&id=<?=$row['id']?>';" > Editar </button>
            <button onclick="location.href='admvideopost.php?act=excluir&id=<?=$row['id']?>';" > Excluir </button>
         </td>
    </tr>
